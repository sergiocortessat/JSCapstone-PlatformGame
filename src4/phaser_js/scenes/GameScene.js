...
export default class GameScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.player = this.physics.add.sprite(gameOptions.playerStartPosition, config.height * 0.7, 'player_run');
  ...
  update() {
    if (this.player.y > config.height) {
      this.scene.start('GameOver', { user: config.user, score: this.score });
    }
